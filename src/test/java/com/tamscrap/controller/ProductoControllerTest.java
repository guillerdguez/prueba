...
public class ProductoControllerTest {
	...
	public void guardarProducto_conAutenticacion_debeRetornarOk() throws Exception {
		...
		Mockito.when(productoService.insertarProducto(producto)).thenReturn(producto);
		ResultActions resultActions = mockMvc.perform(post("/productos/api/productos").contentType(APPLICATION_JSON)
				.content(objectMapper.writeValueAsString(producto))
				.with(user("user1").password("1234").roles("USER", "ADMIN")).with(csrf()));

		resultActions.andExpect(status().isOk()).andExpect(result -> {
			Producto productoResponse = objectMapper.readValue(result.getResponse().getContentAsString(),
					Producto.class);
